...
class ParseOutputPytest:
    ...
    def get_dict_name_with_results(self, name_file_with_analytics_from_test: str) -> dict:
        """The input is a file that is obtained as a result of the
            `start_test` method.
        ...
        list_tests = data['report']['tests']
        dict_test_name_result = {}
        for test in list_tests:
            if test["outcome"] == "failed":
                # Get the output of pytest for a specific test
                result = test['call']['longrepr']
                test_name = 'failed - ' + test['name']
            elif test["outcome"] == "passed":
                result = "passed"
                test_name = 'passed - ' + test['name']

            dict_test_name_result[test_name] = result
        return dict_test_name_result
